  ...
  return (
    <div className="flex flex-col h-screen bg-[#FDFDFD] pt-14 pb-24 px-6 overflow-y-auto">
      <header className="mb-8 pt-4 flex items-center justify-between">
        ...
        <div className="bg-black p-3.5 rounded-[1.5rem] shadow-xl text-white transform -rotate-3 hover:rotate-0 transition-transform"><Trophy size={22} className="text-blue-400" /></div>
      </header>
        ...
        <div className="grid gap-5 pb-10">
          {ranking.map((user, index) => {
            ...
            return (
              <div 
                key={user.id} 
                className={`group p-5 rounded-[2.5rem] shadow-sm border border-gray-100 flex items-center gap-5 transition-all active:scale-[0.98] relative overflow-hidden ${style.bg} ${isTop3 ? 'ring-1 ring-black/5 scale-[1.02] -mx-1' : ''}`}
              >
                ...
                <div className={`relative flex-shrink-0 ${isTop3 ? 'w-16 h-16' : 'w-12 h-12'} bg-white rounded-[1.8rem] shadow-lg border-2 border-white flex items-center justify-center overflow-hidden z-10 transition-all group-hover:scale-105`}>
                   {user.avatar_url ? (
                     <img src={user.avatar_url} className="w-full h-full object-cover" />
                   ) : (
                     <User className={isTop3 ? "text-blue-200" : "text-gray-200"} size={isTop3 ? 28 : 22} />
                   )}
                </div>
                ...
                <div className="flex-1 min-w-0 z-10">
                   ...
                   <div className="flex items-center gap-1.5 mt-1 bg-white/50 w-fit px-2 py-0.5 rounded-lg border border-white/80">
                      ...
                      <p className="text-[11px] font-black text-blue-600 tabular-nums uppercase">{formatTime(user.tempo_leitura_total)}</p>
                   </div>
                </div>
